...
class CompetentService {
  ...
  private static readonly BEHAVIOR_FIELDS = [
    "competency_id",
    "description",
    "status",
  ] as const;
  ...
  static async createCompetent(body: unknown) {
    const sanitizedData = this.getItems(body, 3).map((item) =>
      this.competencyPayload(item, true),
    );
  ...
  static async createBehavior(body: unknown) {
    const payload = this.getItems(body, 20).map((item) =>
      this.behaviorPayload(item, true),
    );
